...
@section('page-subtitle', 'Daftar tahun ajaran yang tersedia')

@section('content')
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <h2 class="text-xl font-display font-semibold text-slate-800">Data Tahun Ajaran</h2>
    <a href="{{ route('admin.tahun-ajaran.create') }}" class="bg-primary text-white font-semibold text-sm px-4 py-2.5 rounded-xl hover:bg-navy transition flex items-center gap-2">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500">
                    <th class="px-6 py-4 font-medium">Nama Tahun Ajaran</th>
            ...
            <tbody class="divide-y divide-slate-100 text-sm">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    <td class="px-6 py-4 font-medium text-slate-700">{{ $ta->nama }}</td>
                    <td class="px-6 py-4 text-slate-700">Semester {{ $ta->semester }}</td>
                    <td class="px-6 py-4 text-slate-700">
                        ...
                        @else
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-700">Tidak Aktif</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-slate-700">{{ $ta->tanggal_mulai->format('d M Y') }}</td>
                    <td class="px-6 py-4 text-slate-700">{{ $ta->tanggal_selesai->format('d M Y') }}</td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.tahun-ajaran.show', $ta->id) }}" class="p-1.5 text-sky hover:text-navy hover:bg-pale rounded-lg transition" title="Lihat">
                            ...
                            <a href="{{ route('admin.tahun-ajaran.edit', $ta->id) }}" class="p-1.5 text-amber-500 hover:text-amber-700 hover:bg-amber-50 rounded-lg transition" title="Edit">
                            ...
                            <form action="{{ route('admin.tahun-ajaran.destroy', $ta->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data tahun ajaran ini?');">
                                @csrf
                                @method('DELETE')
