...
@section('page-subtitle', 'Informasi lengkap data ')
...
<div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 max-w-4xl">
    ...
    <div class="mt-8 pt-6 border-t border-slate-100 flex items-center gap-3">
        <a href="{{ route('admin.data.edit', $data->id) }}" class="bg-primary text-white font-semibold text-sm px-6 py-2.5 rounded-xl hover:bg-navy transition">Edit Data</a>
        <a href="{{ route('admin.data.index') }}" class="border border-slate-200 text-slate-600 font-semibold text-sm px-6 py-2.5 rounded-xl hover:bg-slate-50 transition">Kembali ke Daftar</a>
    </div>
</div>
@endsection
