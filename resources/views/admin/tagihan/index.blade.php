...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Data Tagihan SPP</h2>
            <p class="text-slate-500 mt-1">Daftar seluruh tagihan SPP siswa (hanya lihat)</p>
        </div>
        <a href="{{ route('admin.tagihan.cetak-pdf', request()->query()) }}" target="_blank" class="inline-flex items-center justify-center bg-slate-700 text-white font-semibold text-sm px-6 py-2.5 rounded-xl hover:bg-slate-800 transition whitespace-nowrap">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-4 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('admin.tagihan.index') }}" method="GET" class="flex flex-wrap gap-4">
