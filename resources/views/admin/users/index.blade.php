...
@section('page-subtitle', 'Daftar semua pengguna sistem')

@section('content')
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <h2 class="text-xl font-display font-semibold text-slate-800">Data User</h2>
    <a href="{{ route('admin.users.create') }}" class="bg-primary text-white font-semibold text-sm px-4 py-2.5 rounded-xl hover:bg-navy transition flex items-center gap-2">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 text-sm">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4 text-slate-700">{{ $user['username'] }}</td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.users.show', $user['id']) }}" class="p-1.5 text-sky hover:text-navy hover:bg-pale rounded-lg transition" title="Lihat">
                            ...
                            <a href="{{ route('admin.users.edit', $user['id']) }}" class="p-1.5 text-amber-500 hover:text-amber-700 hover:bg-amber-50 rounded-lg transition" title="Edit">
                            ...
                            <form action="#" method="POST" class="inline" onsubmit="return confirm('Hapus data user ini?');">
                                @csrf
                                @method('DELETE')
