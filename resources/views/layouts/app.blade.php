    ...
    <nav class="flex-1 px-3 py-6 space-y-1 text-sm overflow-y-auto">
      ...
      @if(Auth::user()->isAdmin())
        ...
        <a href="{{ route('admin.tahun-ajaran.index') }}" class="relative flex items-center gap-3 px-3 py-2.5 rounded-xl {{ Str::startsWith($currentRoute, 'admin.tahun-ajaran') ? 'bg-white/10 font-medium' : 'text-sky/90 hover:bg-white/10 hover:text-white transition' }}">
          @if(Str::startsWith($currentRoute, 'admin.tahun-ajaran'))
            <span class="absolute left-0 top-1.5 bottom-1.5 w-1 rounded-full bg-primary"></span>
          @endif
        ...
        {{-- <a href="{{ route('admin.data.index') }}" class="relative flex items-center gap-3 px-3 py-2.5 rounded-xl {{ Str::startsWith($currentRoute, 'admin.tahun-ajaran') ? 'bg-white/10 font-medium' : 'text-sky/90 hover:bg-white/10 hover:text-white transition' }}">
          @if(Str::startsWith($currentRoute, 'admin.data'))
            <span class="absolute left-0 top-1.5 bottom-1.5 w-1 rounded-full bg-primary"></span>
          @endif
          <svg viewBox="0 0 24 24" class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
         data
